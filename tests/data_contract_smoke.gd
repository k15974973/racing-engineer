...
func _run() -> void:
	...
	for key in ["blocks", "inductions", "materials", "tracks", "unlocks"]:
	...
	print("DATA_CONTRACT_SMOKE_OK blocks=%s inductions=%s materials=%s tracks=%s unlocks=%s health=%s" % [summary.get("blocks", 0), summary.get("inductions", 0), summary.get("materials", 0), summary.get("tracks", 0), summary.get("unlocks", 0), actual_health])
	quit(0)
