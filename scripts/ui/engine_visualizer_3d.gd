...
func _build_induction(induction_id: String, intake_scale: float, forced: bool) -> void:
	if not forced:
		_engine_root.add_child(_cylinder("throttle_body", Vector3(-1.75, 0.72, 0.78), 0.16 * intake_scale, 0.26, AIR_COLOR, Vector3(0.0, 0.0, 90.0)))
		return
	...
	for i in turbo_count:
		...
		if turbo_count == 1:
			z = -0.86
		var turbo := _cylinder("turbo_%s" % i, Vector3(1.42, 0.34, z), 0.18 * intake_scale, 0.16, Color(0.80, 0.86, 0.92, 0.76), Vector3(90.0, 0.0, 0.0))
